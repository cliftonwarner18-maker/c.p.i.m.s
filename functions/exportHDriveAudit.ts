...
Deno.serve(async (req) => {
  try {
    ...
    const inventoryRows = drives.map((drive, idx) => {
      ...
      return `
        <tr style="${seized ? 'background-color: #ffe6b4; border-left: 3px solid #c86400;' : idx % 2 === 0 ? 'background-color: #f0f4fc;' : 'background-color: white;'}">
          ...
          <td style="padding: 5px 4px; font-size: 12px; border: 1px solid #b4b4c8;">${(drive.make || '') + ' ' + (drive.model || '')}`.trim() || '-' + '</td>
    ...
    const seizedRows = seizedDrives.map(d => `
      <tr style="background-color: #ffebb8;">
      ...
      ${d.seizure_notes ? `<tr style="background-color: #fffadc;"><td colspan="6" style="padding: 4px; font-size: 11px; border: 1px solid #c86400; color: #783c00; font-style: italic;">NOTE: ${d.seizure_notes.substring(0, 120)}</td></tr>` : ''}
    `).join('');
    ...
    const html = `
      ...
      <head>
        <meta charset="UTF-8">
        <title>H-Drive Audit Report</title>
        <style>
          ...
          .seized-header { background: #c86400; color: white; }
      ...
      <body>
        ...
        <div class="meta">
          Generated: ${moment().tz('America/New_York').format('MM/DD/YYYY HH:mm')} | Records: ${drives.length}${filterParts.length ? ' | Filters: ' + filterParts.join(', ') : ''}
        </div>
        ...
        ${seizedRows ? `
          <div class="section-title" style="background: #c86400;">*** SEIZED / LEGAL HOLD DRIVES ***</div>
          <table>
            <thead>
              <tr class="seized-header">
                <th>SERIAL #</th>
                <th>SEIZING AGENCY</th>
                <th>SEIZING PERSON</th>
                <th>CASE #</th>
                <th>DATE/TIME</th>
                <th>REASON</th>
              </tr>
        ...
        <div class="footer-box">
          ...
          <div class="footer-text">
            By signing below, the undersigned acknowledges receipt and current custody of the H-Drive unit(s) listed above. The signing party accepts full personal responsibility for the security, safekeeping, and proper handling of all assigned drives. The signing party is fully liable for any loss, theft, or intentional damage to assigned H-Drive units. By signing, the undersigned agrees to immediately report any loss or suspected theft to their direct supervisor and the Transportation Department.
          </div>
          <div style="margin-top: 20px;">
            <div style="display: inline-block; margin-right: 20px;">
              <div class="sig-line"></div>
              <div class="sig-label">RESPONSIBLE PARTY SIGNATURE</div>
            </div>
            <div style="display: inline-block; margin-right: 20px;">
              <div class="sig-line"></div>
