...
function formatTime(isoStr) {
  if (!isoStr) return '';
  return moment(isoStr).format('MM/DD/YYYY HH:mm');
}

function toLocalDatetimeValue(isoStr) {
  if (!isoStr) return '';
  return moment(isoStr).format('YYYY-MM-DDTHH:mm');
}
...
export default function WorkOrderDetail() {
  ...
  const [liveElapsed, setLiveElapsed] = useState(0); // minutes, live ticking
  ...
  useEffect(() => {
    if (timerRunning) {
      timerRef.current = setInterval(() => {
        setLiveElapsed(prev => prev + (1 / 60)); // +1 second in minutes
      }, 1000);
    } else {
  ...
  const handleStopTimer = () => {
    ...
    const elapsed = start ? Math.round((new Date() - new Date(start)) / 60000) : Math.round(liveElapsed);
  ...
  const handleComplete = () => {
    ...
    if (timerRunning) {
      setTimerRunning(false);
      elapsed = form.repair_start_time ? Math.round((new Date() - new Date(form.repair_start_time)) / 60000) : Math.round(liveElapsed);
    }
  ...
  const handleExportPDF = async () => {
    ...
    // Signature lines
    if (y + 80 > doc.internal.pageSize.getHeight() - 40) {
      doc.addPage();
      y = 40;
    }
  ...
  if (isLoading || !form) return <LoadingScreen isLoading={true} message="LOADING WORK ORDER..." />;
  ...
  const displayElapsed = timerRunning
    ? Math.round((form.elapsed_time_minutes || 0) + liveElapsed)
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', fontFamily: FF }}>
      ...
      {/* ── PART 1: Initial Complaint (read-only) ── */}
      ...
      <div style={{ background: 'white', border: '1px solid hsl(220,18%,78%)', borderRadius: '2px', padding: '14px' }}>
        ...
        <div style={rowStyle}>
          <div style={fieldStyle}>
            <label style={labelStyle}>REPAIR START TIME (24H)</label>
            <div style={{ display: 'flex', gap: '4px' }}>
              <input
                ...
                readOnly
                placeholder="—"
                style={{ ...inputStyle, flex: 1, background: 'hsl(220,10%,96%)', color: 'hsl(220,10%,35%)' }}
              />
              <button
                onClick={() => { const now = new Date().toISOString(); setForm({ ...form, repair_start_time: now }); }}
                title="Use current time"
                style={{ padding: '4px 8px', fontSize: '10px', fontFamily: FF, background: 'hsl(220,55%,38%)', color: 'white', border: 'none', borderRadius: '2px', cursor: 'pointer', whiteSpace: 'nowrap', fontWeight: '700' }}
              >NOW</button>
            </div>
          </div>
          <div style={fieldStyle}>
            <label style={labelStyle}>REPAIR END TIME (24H)</label>
            <div style={{ display: 'flex', gap: '4px' }}>
              <input
                type="text"
          ...
          <div style={{ minWidth: '140px' }}>
            <label style={labelStyle}>ELAPSED TIME TOTAL</label>
        ...
        <div style={{ ...rowStyle, marginTop: '4px' }}>
          <div style={fieldStyle}>
            ...
            <input
              ...
              placeholder="Enter technician / mechanic name"
              style={inputStyle}
            />
          </div>
        </div>
        ...
        <div style={{ marginBottom: '12px' }}>
          ...
          <textarea
            ...
            placeholder="Describe all repairs and remedies performed in detail..."
            rows={7}
