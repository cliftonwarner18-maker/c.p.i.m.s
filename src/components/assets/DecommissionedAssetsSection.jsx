        ...
        {bulkModifyMode && selectedIds.size > 0 && (
          <div style={{ background: 'hsl(200,80%,92%)', border: '1px solid hsl(200,80%,70%)', borderRadius: '2px', padding: '10px', marginBottom: '8px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
            ...
            <div style={{ display: 'flex', gap: '8px', alignItems: 'flex-end', flexWrap: 'wrap' }}>
              ...
              <button onClick={() => { const updates = {}; if (bulkData.decom_status?.trim()) updates.decom_status = bulkData.decom_status; if (bulkData.current_location?.trim()) updates.current_location = bulkData.current_location.trim(); if (bulkData.out_of_inventory === true) updates.out_of_inventory = true; bulkUpdateMutation.mutate(updates); }} disabled={(!bulkData.decom_status?.trim() && !bulkData.current_location?.trim() && bulkData.out_of_inventory !== true) || bulkUpdateMutation.isPending} style={{ ...btnBase, background: 'hsl(140,55%,38%)', color: 'white', borderColor: 'hsl(140,55%,30%)' }}>Apply</button>
