    ...
    <div style={{ border: '1px solid hsl(220,18%,78%)', borderRadius: '2px', overflow: 'hidden', fontFamily: FF }}>
      ...
      <div style={{ padding: '10px', background: 'hsl(220,10%,98%)', display: 'flex', flexDirection: 'column', gap: '8px' }}>
        <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', alignItems: 'center' }}>
          <button onClick={() => { setEditingAsset(null); setFormData({}); setShowForm(true); }} style={{ ...btnBase, background: 'hsl(0,60%,42%)', color: 'white', borderColor: 'hsl(0,60%,35%)' }}><Plus style={{ width: 12, height: 12 }} /> Log Decommission</button>
        ...
        {bulkModifyMode && selectedIds.size > 0 && (
          <div style={{ background: 'hsl(200,80%,92%)', border: '1px solid hsl(200,80%,70%)', borderRadius: '2px', padding: '10px', marginBottom: '8px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
            ...
            <div style={{ display: 'flex', gap: '8px', alignItems: 'flex-end', flexWrap: 'wrap' }}>
              ...
              <button onClick={() => { const updates = {}; if (bulkData.decom_status && bulkData.decom_status !== '') updates.decom_status = bulkData.decom_status; if (bulkData.current_location && bulkData.current_location.trim() !== '') updates.current_location = bulkData.current_location.trim(); if (bulkData.out_of_inventory === true) updates.out_of_inventory = true; if (Object.keys(updates).length > 0) bulkUpdateMutation.mutate(updates); }} disabled={bulkUpdateMutation.isPending} style={{ ...btnBase, background: 'hsl(140,55%,38%)', color: 'white', borderColor: 'hsl(140,55%,30%)' }}>Apply</button>
